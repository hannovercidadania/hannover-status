# Status dos sistemas

**🟢 Todos os sistemas de pé.**
Última verificação: 17/08/2026, 08:11:46 (Brasília).

| Sistema | Agora | Resposta | 24 h | 7 dias |
|---|---|---|---|---|
| CRM | 🟢 de pé | 200 · 3343 ms | 93.9% | 97.4% |
| Site Hannover | 🟢 de pé | 200 · 267 ms | 100.00% | 100.00% |
| Site Viena | 🟢 de pé | 200 · 492 ms | 100.00% | 100.00% |
| Site German Citizenship | 🟢 de pé | 200 · 137 ms | 100.00% | 100.00% |
| Site Werlang | 🟢 de pé | 200 · 315 ms | 100.00% | 100.00% |

## Peças do CRM na última medição

- 🟢 **Banco de dados** — 1022 ms
- 🟢 **Inbox** — 796 ms
- 🟢 **Anexos** — 396 ms
- 🟢 **WhatsApp (Z-API)**
- 🟢 **Conta Azul**
- 🟢 **Clicksign**
- 🟢 **ClickUp (processos)**
- 🟢 **Instagram e Messenger**
- 🟢 **Tabela de valores (site x CRM)**

---
Verificado a cada 30 minutos por um vigia que roda no GitHub, fora da Vercel e fora
da Supabase. Quando um sistema crítico está fora, este job falha de propósito e o
GitHub manda e-mail. Nenhum dado de cliente aparece aqui.
