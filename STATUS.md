# Status dos sistemas

**🟢 Todos os sistemas de pé.**
Última verificação: 11/08/2026, 01:07:30 (Brasília).

| Sistema | Agora | Resposta | 24 h | 7 dias |
|---|---|---|---|---|
| CRM | 🟢 de pé | 200 · 3457 ms | 78.3% | 92.5% |
| Site Hannover | 🟢 de pé | 200 · 274 ms | 100.00% | 100.00% |
| Site Viena | 🟢 de pé | 200 · 750 ms | 100.00% | 100.00% |
| Site German Citizenship | 🟢 de pé | 200 · 412 ms | 100.00% | 100.00% |
| Site Werlang | 🟢 de pé | 200 · 312 ms | 100.00% | 100.00% |

## Peças do CRM na última medição

- 🟢 **Banco de dados** — 996 ms
- 🟢 **Inbox** — 717 ms
- 🟢 **Anexos** — 196 ms
- 🟢 **WhatsApp (Z-API)**
- 🟢 **Conta Azul**
- 🟢 **Clicksign**
- 🟢 **ClickUp (processos)**
- 🟢 **Instagram e Messenger**

---
Verificado a cada 30 minutos por um vigia que roda no GitHub, fora da Vercel e fora
da Supabase. Quando um sistema crítico está fora, este job falha de propósito e o
GitHub manda e-mail. Nenhum dado de cliente aparece aqui.
