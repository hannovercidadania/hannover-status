# Status dos sistemas

**🟢 Todos os sistemas de pé.**
Última verificação: 17/08/2026, 05:07:30 (Brasília).

| Sistema | Agora | Resposta | 24 h | 7 dias |
|---|---|---|---|---|
| CRM | 🟢 de pé | 200 · 4008 ms | 92.3% | 96.5% |
| Site Hannover | 🟢 de pé | 200 · 67 ms | 100.00% | 100.00% |
| Site Viena | 🟢 de pé | 200 · 448 ms | 100.00% | 100.00% |
| Site German Citizenship | 🟢 de pé | 200 · 56 ms | 100.00% | 100.00% |
| Site Werlang | 🟢 de pé | 200 · 199 ms | 100.00% | 100.00% |

## Peças do CRM na última medição

- 🟢 **Banco de dados** — 1002 ms
- 🟢 **Inbox** — 741 ms
- 🟢 **Anexos** — 153 ms
- 🟡 **WhatsApp (Z-API)**
- 🟢 **Conta Azul**
- 🟢 **Clicksign**
- 🟢 **ClickUp (processos)**
- 🟢 **Instagram e Messenger**
- 🟢 **Tabela de valores (site x CRM)**

---
Verificado a cada 30 minutos por um vigia que roda no GitHub, fora da Vercel e fora
da Supabase. Quando um sistema crítico está fora, este job falha de propósito e o
GitHub manda e-mail. Nenhum dado de cliente aparece aqui.
