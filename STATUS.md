# Status dos sistemas

**🟢 Todos os sistemas de pé.**
Última verificação: 14/08/2026, 04:26:54 (Brasília).

| Sistema | Agora | Resposta | 24 h | 7 dias |
|---|---|---|---|---|
| CRM | 🟢 de pé | 200 · 3856 ms | 100.00% | 93.9% |
| Site Hannover | 🟢 de pé | 200 · 284 ms | 100.00% | 100.00% |
| Site Viena | 🟢 de pé | 200 · 509 ms | 100.00% | 100.00% |
| Site German Citizenship | 🟢 de pé | 200 · 198 ms | 100.00% | 100.00% |
| Site Werlang | 🟢 de pé | 200 · 415 ms | 100.00% | 100.00% |

## Peças do CRM na última medição

- 🟢 **Banco de dados** — 722 ms
- 🟢 **Inbox** — 699 ms
- 🟢 **Anexos** — 178 ms
- 🟡 **WhatsApp (Z-API)**
- 🟢 **Conta Azul**
- 🟢 **Clicksign**
- 🟢 **ClickUp (processos)**
- 🟢 **Instagram e Messenger**

---
Verificado a cada 30 minutos por um vigia que roda no GitHub, fora da Vercel e fora
da Supabase. Quando um sistema crítico está fora, este job falha de propósito e o
GitHub manda e-mail. Nenhum dado de cliente aparece aqui.
